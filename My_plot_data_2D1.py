...
if __name__ == "__main__":
    ...
    for i in range(stop_state_log):
        plt.clf()
        base_pos = np.array([loaded_dict['base_pos_x'][i],
                             loaded_dict['base_pos_y'][i],
                             loaded_dict['base_pos_z'][i]])
        base_quat = np.array([loaded_dict['base_quat_0'][i],
                              loaded_dict['base_quat_1'][i],
                              loaded_dict['base_quat_2'][i],
                              loaded_dict['base_quat_3'][i]])
        ...
        plt.plot([-0.5, 10], [0, 0], 'k', alpha=0.5)
        plt.xlim([-1.0279631452507143, 10.587226050264999])
        plt.ylim([-0.3745306139430682, 1.1745747929057757])
        # plt.axis('equal')
        # plt.ylim([-0.1, 1])
